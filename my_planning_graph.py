...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        for eff in iter(actionA.effects):
                if ~eff in actionB.effects:
                    return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        for precon in iter(actionA.preconditions):
            if ~precon in actionB.effects:
                return True
        for precon in iter(actionB.preconditions):
            if ~precon in actionA.effects:
                return True
        return False
...
class PlanningGraph:
    ...
    def h_maxlevel(self):
        ...
        for goal in self.goal:
            for i,layer in enumerate(self.literal_layers):
                if goal in layer:
                    layer_with_goal= max(layer_with_goal,i)
                    break
        return layer_with_goal
    ...
    def h_setlevel(self):
        ...
        def check_mutex(self):
            '''return true if no mutex is found'''
            for goal1,goal2 in itertools.combinations(self.goal,2):
        ...
        while not self._is_leveled:
            self.fill(1)
            if check_goals(self) and check_mutex(self):
                self._is_leveled = True
            level +=1
        return level
